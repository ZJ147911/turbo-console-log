...
import { insertCustomLogCommand } from '@/commands/insertCustomLog';
...
describe('insertCustomLogCommand', () => {
  ...
  it('should call debugMessage.msg with custom log function from extensionProperties', async () => {
  ...
  it('should default to "log" when no logFunction is defined in extensionProperties', async () => {
    const mockDocument = makeTextDocument(['const myVar = 42;']);
  ...
  it('should handle multiple selections with custom log function', async () => {
  ...
  describe('PHP Pro-only blocking', () => {
    ...
    it('should insert log when canInsertLogInDocument returns true (JS/TS or Pro user)', async () => {
      ...
      expect(debugMessage.msg).toHaveBeenCalledWith(
        ...
        2,
        {},
        'log', // Defaults to 'log' when no logFunction in extensionProperties
      );
      expect(mockTrackNewUserJourney).toHaveBeenCalledWith(context);
    });
  });
